...
最終更新: 2026-07-28（Issue #123 SQS consumer PoCを反映）
...
- Queue transport: HTTP通知、SQS SendMessage / ReceiveMessage / ChangeMessageVisibility / DeleteMessage
...
7. `API_QUEUE_TRANSPORT=sqs`ではSQS producerが`SendMessage`する。
8. `WORKER_QUEUE_CONSUMER=http`ではWorkerが`POST /jobs`を共通message contractで検証する。
9. `WORKER_QUEUE_CONSUMER=sqs`ではWorkerが`ReceiveMessage`で一件ずつlong pollingする。
...
- `maxReceiveCount=5`とDLQ retention 14日は初期候補である。
...
- 対象queueへの`sqs:SendMessage`だけを基本権限とする。
- Customer managed KMS keyでは`kms:Decrypt` / `kms:GenerateDataKey`を追加する。

### Consumer

- 対象queueへの`sqs:ReceiveMessage` / `sqs:DeleteMessage` / `sqs:ChangeMessageVisibility`だけを基本権限とする。
- Customer managed KMS keyでは対象keyへの`kms:Decrypt`を追加する。
...
IAM role、queue policy、KMS key policy、TLS強制、VPC endpointはdeployment / IaC責務とする。
...
- 実AWS source queue / DLQ / RedrivePolicy / IAM role / deploymentは未作成・未検証。
