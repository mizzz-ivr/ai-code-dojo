...
- Issue #127でstaging専用GitHub OIDC trust、deployment role、CloudFormation execution role、review-only change set workflowを実装中。
- Issue #127の設計・実装境界はNotionへ同期済み。
...
- `infra/aws/cloudformation/github-oidc-deployment-role-stack.json`を追加する。
...
- GitHub Actions deployment roleとCloudFormation execution roleを分離する。
...
- `.github/workflows/deploy-sqs-staging-change-set.yml`は`workflow_dispatch` / main / GitHub Environment `staging`専用とする。
...
- Security regression unit testを7件追加する。
...
Issue #125 / PR #126のfinal code / docs headで以下は成功済み。
...
Issue #127の作業時点では以下をローカル確認済み。

- OIDC deployment validator unit test: 7 / 7 success
- OIDC deployment CloudFormation JSON parse: Success
- Review-only workflow security contract: Success
...
- Issue #127の作業branchは `feat/staging-oidc-change-set`。
